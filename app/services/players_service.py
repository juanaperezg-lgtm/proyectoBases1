    ...
    with get_connection() as conn:
        ...
        try:
            cursor.execute(
                ...
                """,
                (nombre_completo, posicion, fecha_nacimiento, estatura_m, peso_kg, valor_mercado, id_equipo),
            )
            conn.commit()
        finally:
            cursor.close()
    ...
    with get_connection() as conn:
        ...
        try:
            cursor.execute(
                ...
                WHERE id_jugador = %s
                """,
                (nombre_completo, posicion, fecha_nacimiento, estatura_m, peso_kg, valor_mercado, id_equipo, id_jugador),
            )
            conn.commit()
        finally:
            cursor.close()
